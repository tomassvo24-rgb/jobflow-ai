  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div>
            ...
            {profile.field && (
              <div className="flex flex-wrap gap-2 mt-4">
                <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{ background: "rgba(255,255,255,0.10)", color: "rgba(255,255,255,0.85)" }}>📋 {profile.field}</span>
                {profile.city && <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{ background: "rgba(255,255,255,0.10)", color: "rgba(255,255,255,0.85)" }}>📍 {profile.city}</span>}
                {profile.level && <span className="px-3 py-1 rounded-full text-xs font-semibold" style={{ background: "rgba(255,255,255,0.10)", color: "rgba(255,255,255,0.85)" }}>🎓 {profile.level}</span>}
              </div>
            )}
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="shrink-0 px-7 py-3.5 rounded-2xl text-sm font-bold text-white transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            style={{
      ...
      {loading && (
        ...
        >
          <div className="text-4xl mb-3" style={{ animation: "pulse 1.4s ease-in-out infinite" }}>🤖</div>
      ...
      {/* ── Error ── */}
      {error && (
        <div className="rounded-2xl border p-4 text-sm" style={{ background: "#fff1f2", borderColor: "#fecdd3", color: "#be123c" }}>
          ⚠️ {error}
        </div>
      )}
      ...
      {runMeta && !loading && (
        ...
        >
          <span style={{ color: "#6b7280" }}>✅ Zdroje: <strong style={{ color: "#0d1b2a" }}>{runMeta.sources_checked?.join(", ")}</strong></span>
          <span style={{ color: "#6b7280" }}>📄 Nalezeno: <strong style={{ color: "#0d1b2a" }}>{runMeta.total_found}</strong></span>
          <span style={{ color: "#6b7280" }}>🎯 Zobrazeno: <strong style={{ color: "#0d1b2a" }}>{runMeta.filtered_to}</strong></span>
          {runMeta.sources_failed?.length > 0 && (
            <span style={{ color: "#d97706" }}>⚠️ Chyba: {runMeta.sources_failed.join(", ")}</span>
          )}
        </div>
      )}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="lg:hidden mb-3 flex items-center justify-between">
              ...
              <button
                ...
                className="text-sm px-3 py-1.5 rounded-xl border font-semibold transition-colors"
